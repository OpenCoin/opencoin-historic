...
def decodeFraction(s):
    from fraction import Fraction
    return Fraction(s)

def encodeFractionList(l):
    return [str(f) for f in l]

def decodeFractionList(l):
...
class CurrencyDescriptionDocument(ContainerWithSignature):
    ...
    codecs = {'issuer_cipher_suite':{'encode':encodeCryptoContainer,'decode':decodeCryptoContainer},
              ...
              'denominations':{'encode':encodeFractionList, 'decode':decodeFractionList},}
...
class MintKey(ContainerWithSignature):
    ...
    codecs = {'key_identifier':{'encode':base64.b64encode,'decode':base64.b64decode},
              ...
              'denomination':{'encode':str,'decode':decodeFraction},}
...
class CurrencyBase(Container):
    ...
    def __add__(self,other):
        ...
        if type(other) == type(self) and self.sameCurrency(other):
            return val + other.denomination
        elif isinstance(other, Fraction):
            return val + other
        else:
            raise Exception("type other: %s, type self: %s" % (type(other), type(self)))
...
def sumCurrencies(_list):
    """sumCurrencies is exactly the same as sum(collection_of_currencies) except it works with complex types."""
    from fraction import Fraction
